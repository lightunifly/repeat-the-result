...
for i in range(2,21):
    print('u'+str(i)+'=u'+str(i-1)+'+tf.reduce_sum(tf.square(deltau'+str(i-1)+'),axis=1)*deltaT+np.sqrt(2.0)*tf.reduce_sum(tf.multiply(deltau'+str(i-1)+',deltaW[:,'+str(i-1)+',:]),axis=1)')
